...
size_t search_chunks(const uint32_t *freq, const size_t lenFreq,
    ...
    while (usually(freq < stopFreq)) {
            
        uint32_t maxChunk = freq[CHUNKINTS - 1];

        if (sometimes(maxChunk < nextMatch)) {
            freq += CHUNKINTS;  
            continue;  
        }  
        ...
        do {
            ...
            // NOTE: the correct vectors usually are already preloaded 
        } while (usually(maxChunk >= nextMatch));

            // exiting inner loop requires further increment of freq
            freq += CHUNKINTS;
    }
    // exit from outer loop means we reached stopFreq
    // FIXME: names
    freq -= CHUNKINTS; // undue the last increment that put us over?
    const uint32_t lenF = lastFreq - freq + 1;
    const uint32_t lenR = lastRare - rare + 1;
    return count + finish_scalar(freq, lenF, rare, lenR);
}
